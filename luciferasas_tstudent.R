...
# comparacion por linea celular de todas las condiciones entre si con una t de student
for (i in 1:length(celulas)){
  ...
  for (j in 1:length(condiciones)){
    for(k in 1:length(condiciones)){
      if (condiciones[k] %in% luci[luci$Celulas==celulas[i],]$Plasmido & 
          condiciones[j] %in% luci[luci$Celulas==celulas[i],]$Plasmido ){
        pvalor <- t.test(luci[luci$Celulas==celulas[i] & luci$Plasmido==condiciones[j],]$RLA_Value,
                         luci[luci$Celulas==celulas[i] & luci$Plasmido==condiciones[k],]$RLA_Value
                         )$p.value
        if (pvalor <= 0.05){
          print(paste0(celulas[i],": ",condiciones[j]," vs ",condiciones[k]," p-valor: ",pvalor))
        }
      }
    }
  }
}
# Hasta aqui funciona pero quisiera que generara un DF y un archivo por cada linea celular... por hacer
